...
const Devices = {
  ...
  getAccountHistory(date, unit='km') {
    this._validateRequiredFields({date, unit});
    let queryString = {
      unit: unit
    }

    return HTRequest.get(DEVICES_BASE_PATH + 'history/' + date + '?' + querystring.stringify(queryString))
  },
